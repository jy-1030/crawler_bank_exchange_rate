...
public class TEJ_219_NDB_YRATE_01 extends PorcessFrame {
	public void taskDescription(Parameter param, IDBConnector market, String[] args) {
		...
		DL_219_NDB_YRATE_01 dl = new DL_219_NDB_YRATE_01(parameter);
		String url_1 = dl.parameter.getMenu().get("url_1");
		String url_2 = dl.parameter.getMenu().get("url_2");
		...
		try {
			// 先抓貨幣清單
			cur_list = dl.getlist(url_1);
		} catch (Exception e) {
			logger.error(ErrorTitle.CONNECT_TITLE.getTitle(), e);
		}
		// 第二段 : 分析資料存入暫存處理
				try {
				logger.info("網址 : " + url_2);
				source = dl.getData(url_2);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static void main(String[] args) {
		...
		try {
			...
			TEJ_219_NDB_YRATE_01 runPg = new TEJ_219_NDB_YRATE_01();
